...
"""Conformance scenarios: anthropic beta chat via ``with_raw_response``.
...
class InferenceBetaRawResponseScenario(Scenario):
        ...
        with mock.patch.dict(os.environ, _api_key_override()):
            with instrument(
                ...
                with vcr.use_cassette(
                    "test_sync_messages_beta_create_with_raw_response.yaml"
                ):
                    raw_response = (
...
class InferenceBetaRawResponseStreamingScenario(Scenario):
    expected_spans = {"chat": 1}
    expected_metrics = (
        "gen_ai.client.operation.duration",
        "gen_ai.client.token.usage",
