...
"""Data-extraction helpers for AgentScope instrumentation.

These helpers translate AgentScope request/response objects into the
``opentelemetry-util-genai`` invocation model. Invocations are always created
through the public :class:`~opentelemetry.util.genai.handler.TelemetryHandler`
factory methods (``inference``/``embedding``/``invoke_local_agent``) — never
...
def _build_tool_definitions(
    ...
    """Extract tool definitions for tracing as semconv tool-definition objects."""
...
def _format_msg_to_parts(msg: Msg) -> dict[str, Any]:
    ...
    try:
        ...
        logger.debug("Error formatting message: %s", e)
        return {
            "role": msg.role,
            "parts": [
                {
...
def _decode_base64(data: Any) -> bytes | None:
...
def _part_dict_to_message_part(part: Dict[str, Any]) -> MessagePart:
    """Convert a part dict into a util-genai message part."""
    part_type = part.get("type")
    if part_type == "text":
        return Text(content=part.get("content", ""))
    ...
    if part_type == "reasoning":
        return Reasoning(content=part.get("content", ""))
    if part_type == "uri":
        return Uri(
            mime_type=part.get("mime_type"),
            modality=part.get("modality", "image"),
            uri=part.get("uri", ""),
        )
    if part_type == "blob":
        content = _decode_base64(part.get("content", ""))
...
def convert_agentscope_messages_to_genai_format(
    ...
    """Convert AgentScope messages to util-genai ``InputMessage`` objects."""
    if not messages:
        return []
    ...
    for msg in messages:
        ...
        converted_parts: list[MessagePart] = [
            _part_dict_to_message_part(part) for part in parts
        ]
...
def get_chatmodel_output_messages(chat_response: Any) -> List[Dict[str, Any]]:
    """Convert ChatResponse to standard output message dict format."""
    try:
        ...
        logger.warning(
            "Error processing ChatResponse to output messages: %s", e
        )
        return [
            {
                "role": "assistant",
...
def convert_chatresponse_to_output_messages(
    ...
    """Convert ChatResponse to util-genai ``OutputMessage`` objects."""
    ...
    for msg_dict in output_dicts:
        ...
        converted_parts = [
            _part_dict_to_message_part(part) for part in parts_dicts
        ]
...
def convert_agent_response_to_output_messages(
    ...
    """Convert agent ``Msg`` response to util-genai ``OutputMessage`` objects."""
    ...
    try:
        ...
        logger.debug("Failed to convert agent response: %s", e)
        return []
...
def start_llm_invocation(
    ...
    if handler.should_capture_content():
        ...
        if messages:
            try:
                ...
                logger.warning(
                    "Failed to convert input messages: %s", e, exc_info=True
                )
    ...
    if not call_kwargs.get("structured_model", False):
        invocation.tool_definitions = _build_tool_definitions(
...
def start_embedding_invocation(
    ...
    """Create and start an embedding invocation from an embedding model call."""
    provider_name = get_embedding_provider_name(call_instance)
    request_model = getattr(call_instance, "model_name", "unknown_model")

    invocation = handler.embedding(provider_name, request_model=request_model)
...
def start_agent_invocation(
    ...
    """Create and start an ``invoke_agent`` invocation from a reply call."""
    provider_name = None
    request_model = None
    ...
    invocation.agent_id = getattr(reply_instance, "id", "unknown")
    invocation.agent_description = (
        inspect.getdoc(reply_instance.__class__) or "No description available"
    )
    ...
    if handler.should_capture_content():
        ...
        if msg:
            try:
                if isinstance(msg, Msg):
                    invocation.input_messages = (
                        convert_agentscope_messages_to_genai_format([msg])
                    )
                ...
                logger.debug("Error converting agent input messages: %s", e)

        sys_prompt = getattr(reply_instance, "sys_prompt", None)
        if sys_prompt:
