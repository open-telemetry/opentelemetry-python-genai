...
Emits one line suitable for $GITHUB_OUTPUT:

  test_matrix={"include": [...]}

Each test entry has `name`, `tox_env`, `python_version`, and `needs_weaver`
when the tox env ends in `-conformance`. Misc tox envs (everything else) are
emitted by hand in misc.yml because each has bespoke per-job logic.
