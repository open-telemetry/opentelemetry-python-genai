...
- `invoke_local_agent(*, agent_name, request_model)` → `LocalAgentInvocation`
- `invoke_remote_agent(provider, *, request_model, server_address, server_port, agent_name)` → `RemoteAgentInvocation`
