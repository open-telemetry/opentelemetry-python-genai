...
class TestMediaHelpers(unittest.TestCase):
    ...
    ``decode_base64`` and ``image_from_url`` only decode inline base64
    payloads when their ``capture_content`` flag (resolved once up the stack)
    is ``True``; with the flag off they short-circuit and return ``None``
    without decoding. The flag is passed explicitly and never read from the
    environment.
    """
    ...
    def test_image_from_url_data_uri_returns_blob(self):
        part = image_from_url(
            "data:image/jpeg;base64,QUJD", capture_content=True
        )
    ...
    def test_image_from_url_data_uri_skips_decode_without_capture_content(
        self,
    ):
        self.assertIsNone(image_from_url("data:image/jpeg;base64,QUJD"))

    def test_image_from_url_http_returns_uri(self):
        part = image_from_url("https://example.com/cat.png")
        self.assertIsInstance(part, Uri)
    ...
    def test_image_from_url_data_uri_no_mime_type(self):
        part = image_from_url("data:;base64,QUJD", capture_content=True)
        self.assertIsInstance(part, Blob)
    ...
    def test_image_from_url_data_uri_malformed_base64_returns_none(self):
        part = image_from_url(
            "data:image/png;base64,not!valid!base64!", capture_content=True
        )
    ...
    def test_decode_base64_valid_returns_bytes(self):
        self.assertEqual(decode_base64("QUJD", capture_content=True), b"ABC")

    def test_decode_base64_valid_with_padding(self):
        self.assertEqual(
            decode_base64("R0lGODlh", capture_content=True), b"GIF89a"
        )

    def test_decode_base64_strips_whitespace_and_newlines(self):
        self.assertEqual(decode_base64("QU\nJD", capture_content=True), b"ABC")
        self.assertEqual(
            decode_base64("  QUJD  ", capture_content=True), b"ABC"
        )
        self.assertEqual(decode_base64("QU JD", capture_content=True), b"ABC")

    def test_decode_base64_malformed_returns_none(self):
        self.assertIsNone(
            decode_base64("not!valid!base64!", capture_content=True)
        )
        self.assertIsNone(decode_base64("@@@@", capture_content=True))
        self.assertIsNone(decode_base64("****", capture_content=True))

    def test_decode_base64_wrong_padding_returns_none(self):
        # Correct base64 alphabet but invalid length/padding.
        self.assertIsNone(decode_base64("QUJ", capture_content=True))
        self.assertIsNone(decode_base64("QQ", capture_content=True))

    def test_decode_base64_empty_returns_empty_bytes(self):
        self.assertEqual(decode_base64("", capture_content=True), b"")

    def test_decode_base64_real_image_round_trips(self):
        self.assertEqual(
            decode_base64(_REAL_PNG_B64, capture_content=True), _REAL_PNG_BYTES
        )

    def test_decode_base64_without_capture_content_returns_none(self):
        self.assertIsNone(decode_base64("QUJD"))
        self.assertIsNone(decode_base64(_REAL_PNG_B64, capture_content=False))

    @patch.dict(
        os.environ,
        {"OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT": "NO_CONTENT"},
    )
    def test_decode_base64_ignores_env_and_uses_flag(self):
        self.assertEqual(decode_base64("QUJD", capture_content=True), b"ABC")
        self.assertEqual(
            decode_base64(_REAL_PNG_B64, capture_content=True), _REAL_PNG_BYTES
        )
        self.assertIsNone(decode_base64("QUJD", capture_content=False))

    def test_image_from_url_real_png_data_uri_returns_blob(self):
        part = image_from_url(
            f"data:image/png;base64,{_REAL_PNG_B64}", capture_content=True
        )
        self.assertIsInstance(part, Blob)
    ...
    def test_image_from_url_real_png_truncated_base64_returns_none(self):
        ...
        part = image_from_url(
            f"data:image/png;base64,{corrupted}", capture_content=True
        )
        self.assertIsNone(part)
