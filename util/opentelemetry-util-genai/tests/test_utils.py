...
class TestShouldEmitEvent(unittest.TestCase):
    ...
    def test_should_emit_event_with_explicit_mode(self):  # pylint: disable=no-self-use
        assert should_emit_event(ContentCapturingMode.NO_CONTENT) is False
        assert should_emit_event(ContentCapturingMode.SPAN_ONLY) is False
        assert should_emit_event(ContentCapturingMode.EVENT_ONLY) is True
        assert should_emit_event(ContentCapturingMode.SPAN_AND_EVENT) is True

        with patch.dict(
            os.environ,
            {"OTEL_INSTRUMENTATION_GENAI_EMIT_EVENT": "true"},
        ):
            assert should_emit_event(ContentCapturingMode.NO_CONTENT) is True
            assert should_emit_event(ContentCapturingMode.SPAN_ONLY) is True

        with patch.dict(
            os.environ,
            {"OTEL_INSTRUMENTATION_GENAI_EMIT_EVENT": "false"},
        ):
            assert should_emit_event(ContentCapturingMode.EVENT_ONLY) is False
            assert (
                should_emit_event(ContentCapturingMode.SPAN_AND_EVENT) is False
            )
