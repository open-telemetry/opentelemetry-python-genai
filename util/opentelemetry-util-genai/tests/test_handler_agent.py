...
class TestLocalAgentInvocation(unittest.TestCase):  # pylint: disable=too-many-public-methods
    ...
    def test_conversation_id_from_constructor(self):
        """Set at construction so it is on the span before any child starts."""
        invocation = self.handler.invoke_local_agent(
            agent_name="Researcher", conversation_id="conv-456"
        )
        assert invocation.span.attributes[GenAI.GEN_AI_CONVERSATION_ID] == (
            "conv-456"
        )
        invocation.stop()
